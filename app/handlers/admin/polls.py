...
def _safe_format_price(amount_kopeks: int) -> str:
    try:
        ...
        logger.error('Failed to format amount', amount_kopeks=amount_kopeks, error=error)
...
async def _safe_delete_message(message: types.Message) -> None:
    try:
        ...
        if 'message to delete not found' in str(error).lower():
            logger.debug('Message already deleted', error=error)
        else:
            logger.warning('Failed to delete message', message_id=message.message_id, error=error)
...
async def _edit_creation_message(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            'Unexpected error updating poll creation message', message_id=message_id, error=error
        )
    return False
    ...
    if chat_id and message_id:
        try:
            ...
            if 'message to delete not found' in error_text:
                ...
                logger.warning('Failed to delete poll creation message', message_id=message_id, error=error)
        except Exception as error:  # pragma: no cover - defensive logging
            logger.error(
                'Unexpected error deleting poll creation message', message_id=message_id, error=error
            )
...
def _render_creation_progress(
    ...
    *,
    ...
    lines: list[str] = ['🗳️ <b>ایجاد نظرسنجی</b>']
    ...
    description_prompt = texts.t(
        ...
        'توضیحات نظرسنجی را وارد کنید. HTML مجاز است.\n/skip را ارسال کنید تا رد شود.',
    )
    ...
    reward_prompt = texts.t(
        ...
        'مبلغ جایزه را وارد کنید. برای غیرفعال کردن جایزه، ۰ ارسال کنید.',
    )
    ...
    if 'reward_enabled' in data:
        if data.get('reward_enabled'):
            ...
            lines.append(texts.t('ADMIN_POLLS_REWARD_DISABLED', 'جایزه غیرفعال است'))
    ...
    question_prompt = texts.t(
        ...
        (
            'سؤال و گزینه‌های پاسخ را وارد کنید.\n'
            'هر خط یک گزینه جداگانه است.\n'
            'خط اول متن سؤال است.\n'
            '/done را ارسال کنید وقتی سؤالات اضافه شدند.'
        ),
    )
...
def _build_polls_keyboard(polls: list[Poll], language: str) -> types.InlineKeyboardMarkup:
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text=texts.t('ADMIN_POLLS_CREATE', '➕ ایجاد نظرسنجی'),
...
def _format_reward_text(poll: Poll, language: str) -> str:
    ...
    return texts.t('ADMIN_POLLS_REWARD_DISABLED', 'جایزه غیرفعال است')
...
def _build_poll_details_keyboard(poll_id: int, language: str) -> types.InlineKeyboardMarkup:
    ...
    return types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_POLLS_SEND', '📤 ارسال'),
                    callback_data=f'poll_send:{poll_id}',
                )
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_POLLS_STATS', '📊 آمار'),
                    callback_data=f'poll_stats:{poll_id}',
                )
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_POLLS_DELETE', '🗑️ حذف'),
                    callback_data=f'poll_delete:{poll_id}',
                )
            ],
            [types.InlineKeyboardButton(text=texts.t('ADMIN_POLLS_BACK', '⬅️ بازگشت به فهرست'), callback_data='admin_polls')],
        ]
    )
...
def _build_target_keyboard(poll_id: int, language: str) -> types.InlineKeyboardMarkup:
    ...
    return types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_ALL', '👥 همه کاربران'),
                    callback_data=f'poll_target:{poll_id}:all',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_ACTIVE', '📱 کاربران فعال'),
                    callback_data=f'poll_target:{poll_id}:active',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_TRIAL', '🎁 آزمایشی'),
                    callback_data=f'poll_target:{poll_id}:trial',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_NO_SUB', '❌ بدون اشتراک'),
                    callback_data=f'poll_target:{poll_id}:no',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_EXPIRING', '⏰ در حال انقضا'),
                    callback_data=f'poll_target:{poll_id}:expiring',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_EXPIRED', '🔚 منقضی شده'),
                    callback_data=f'poll_target:{poll_id}:expired',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_ACTIVE_ZERO', '🧊 فعال ۰ گیگ'),
                    callback_data=f'poll_target:{poll_id}:active_zero',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_BROADCAST_TARGET_TRIAL_ZERO', '🥶 آزمایشی ۰ گیگ'),
                    callback_data=f'poll_target:{poll_id}:trial_zero',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_POLLS_CUSTOM_TARGET', '⚙️ بر اساس معیار'),
...
def _build_custom_target_keyboard(poll_id: int, language: str) -> types.InlineKeyboardMarkup:
    ...
    return types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_TODAY', '📅 امروز'),
                    callback_data=f'poll_custom_target:{poll_id}:today',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_WEEK', '📅 این هفته'),
                    callback_data=f'poll_custom_target:{poll_id}:week',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_MONTH', '📅 این ماه'),
                    callback_data=f'poll_custom_target:{poll_id}:month',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_ACTIVE_TODAY', '⚡ فعال امروز'),
                    callback_data=f'poll_custom_target:{poll_id}:active_today',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_INACTIVE_WEEK', '💤 غیرفعال ۷+ روز'),
                    callback_data=f'poll_custom_target:{poll_id}:inactive_week',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_INACTIVE_MONTH', '💤 غیرفعال ۳۰+ روز'),
                    callback_data=f'poll_custom_target:{poll_id}:inactive_month',
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_REFERRALS', '🤝 از طریق ارجاع'),
                    callback_data=f'poll_custom_target:{poll_id}:referrals',
                ),
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_CRITERIA_DIRECT', '🎯 ثبت‌نام مستقیم'),
...
def _build_send_confirmation_keyboard(poll_id: int, target: str, language: str) -> types.InlineKeyboardMarkup:
    ...
    return types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t('ADMIN_POLLS_SEND_CONFIRM_BUTTON', '✅ ارسال'),
...
async def show_polls_panel(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    lines = [texts.t('ADMIN_POLLS_LIST_TITLE', '🗳️ <b>نظرسنجی‌ها</b>'), '']
    if not polls:
        lines.append(texts.t('ADMIN_POLLS_LIST_EMPTY', 'نظرسنجی وجود ندارد.'))
    else:
        for poll in polls[:10]:
            reward = _format_reward_text(poll, db_user.language)
            lines.append(
                f'• <b>{html.escape(poll.title)}</b> — '
                f'{texts.t("ADMIN_POLLS_QUESTIONS_COUNT", "تعداد سؤالات: {count}").format(count=len(poll.questions))}\n'
                f'  {reward}'
            )
...
async def process_poll_title(
    ...
    if not title:
        error_text = texts.t(
            ...
            '❌ عنوان نمی‌تواند خالی باشد. دوباره امتحان کنید.',
        )
...
async def process_poll_reward(
    ...
    if reward_kopeks is None:
        error_text = texts.t(
            ...
            '❌ مبلغ نامعتبر است. دوباره امتحان کنید.',
        )
...
async def process_poll_question(
    ...
    if len(lines) < 3:
        error_text = texts.t(
            ...
            '❌ یک سؤال و حداقل دو گزینه پاسخ لازم است.',
        )
    ...
    status_message = texts.t(
        ...
        'سؤال افزوده شد: «{question}». سؤال بعدی را اضافه کنید یا /done ارسال کنید.',
...
async def _render_poll_details(poll: Poll, language: str) -> str:
    ...
    lines.append(texts.t('ADMIN_POLLS_QUESTIONS_COUNT', 'تعداد سؤالات: {count}').format(count=len(poll.questions)))

    if poll.questions:
        lines.append('')
        lines.append(texts.t('ADMIN_POLLS_QUESTION_LIST_HEADER', '<b>سؤالات:</b>'))
...
async def start_poll_send(
    ...
    texts = get_texts(db_user.language)
    await callback.message.edit_text(
        texts.t('ADMIN_POLLS_SEND_CHOOSE_TARGET', '🎯 مخاطبان ارسال نظرسنجی را انتخاب کنید:'),
...
async def show_custom_target_menu(
    ...
    await callback.message.edit_text(
        get_texts(db_user.language).t(
            ...
            'معیار اضافی مخاطبان را انتخاب کنید:',
        ),
...
async def _show_send_confirmation(
    ...
    confirmation_text = texts.t(
        ...
        '📤 ارسال نظرسنجی «{title}» به «{audience}»؟ تعداد کاربران: {count}',
...
async def confirm_poll_send(
    ...
    await callback.message.edit_text(
        texts.t('ADMIN_POLLS_SENDING', '📤 در حال شروع ارسال نظرسنجی...'),
        parse_mode='HTML',
    )
...
async def show_poll_stats(
    ...
    lines = [texts.t('ADMIN_POLLS_STATS_HEADER', '📊 <b>آمار نظرسنجی</b>'), '']
    lines.append(
        texts.t(
            'ADMIN_POLLS_STATS_OVERVIEW',
            'کل دعوت‌شدگان: {total}\nتکمیل کردند: {completed}\nجوایز پرداخت‌شده: {reward}',
        ).format(
...
async def confirm_poll_delete(
    ...
    await callback.message.edit_text(
        texts.t(
            ...
            'آیا مطمئن هستید که می‌خواهید نظرسنجی «{title}» را حذف کنید؟',
...
async def delete_poll_handler(
    ...
    if success:
        await callback.message.edit_text(
            texts.t('ADMIN_POLLS_DELETED', '🗑️ نظرسنجی حذف شد.'),
