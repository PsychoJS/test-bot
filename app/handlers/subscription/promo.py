...
async def claim_discount_offer(
    ...
    try:
        ...
        await callback.answer(
            texts.get('DISCOUNT_CLAIM_NOT_FOUND', '❌ پیشنهاد پیدا نشد'),
            show_alert=True,
        )
        return
    ...
    if offer.claimed_at is not None:
        await callback.answer(
            texts.get('DISCOUNT_CLAIM_ALREADY', 'ℹ️ تخفیف قبلاً فعال شده است'),
            show_alert=True,
        )
        return
    ...
    if not offer.is_active or offer.expires_at <= now:
        ...
        await db.commit()
        await callback.answer(
            texts.get('DISCOUNT_CLAIM_EXPIRED', '⚠️ مدت پیشنهاد منقضی شده است'),
            show_alert=True,
        )
        return
    ...
    if effect_type == 'test_access':
        ...
        if not success:
            if error_code == 'subscription_missing':
                error_message = texts.get(
                    ...
                    '❌ برای فعال‌سازی پیشنهاد، اشتراک فعال لازم است.',
                )
            elif error_code == 'squads_missing':
                error_message = texts.get(
                    'TEST_ACCESS_NO_SQUADS',
                    '❌ لیست سرورهای آزمایشی یافت نشد. با مدیر تماس بگیرید.',
                )
            elif error_code == 'already_connected':
                error_message = texts.get(
                    'TEST_ACCESS_ALREADY_CONNECTED',
                    'ℹ️ این سرور قبلاً به اشتراک شما متصل شده است.',
                )
            elif error_code == 'remnawave_sync_failed':
                error_message = texts.get(
                    'TEST_ACCESS_REMNAWAVE_ERROR',
                    '❌ اتصال سرورها ناموفق بود. بعداً امتحان کنید یا با پشتیبانی تماس بگیرید.',
                )
            else:
                error_message = texts.get(
                    'TEST_ACCESS_UNKNOWN_ERROR',
                    '❌ فعال‌سازی پیشنهاد ناموفق بود. بعداً امتحان کنید.',
                )
        ...
        success_message = texts.get(
            ...
            '🎉 سرورهای آزمایشی متصل شدند! دسترسی تا {expires_at} فعال است.',
        ).format(expires_at=expires_text)

        popup_text = texts.get('TEST_ACCESS_ACTIVATED_POPUP', '✅ دسترسی اعطا شد!')
        await callback.answer(popup_text, show_alert=True)
        back_keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.get('BACK_TO_MENU', '🏠 به منوی اصلی'),
    ...
    if discount_percent <= 0:
        await callback.answer(
            texts.get('DISCOUNT_CLAIM_ERROR', '❌ فعال‌سازی تخفیف ناموفق بود. بعداً امتحان کنید.'),
            show_alert=True,
        )
        return
    ...
    success_template = texts.get(
        ...
        '🎉 تخفیف {percent}% فعال شد! در پرداخت بعدی اعمال می‌شود.',
    )
    ...
    await callback.answer('✅ تخفیف فعال شد!', show_alert=True)
    ...
    if offer_type == 'purchase_discount':
        button_text = texts.get('MENU_BUY_SUBSCRIPTION', '💎 خرید اشتراک')
        button_callback = 'subscription_upgrade'
    elif offer_type == 'extend_discount':
        button_text = texts.get('SUBSCRIPTION_EXTEND', '💎 تمدید اشتراک')
